...
def preprocess_data(df, target_col):
    ...
    # Scale numeric features (good for Logistic Regression)
...
def pretty_loan_output(raw_pred):
    """
    Map raw label to friendly message.
    Dataset uses Loan_Status = 'Yes' / 'No'
    """
    s = str(raw_pred)
    if s == "Yes":
...
def prediction_form(df, meta, model):
    ...
    with st.form("prediction_form"):
        ...
        for col in meta["original_columns"]:
            ...
            if col in meta["numeric_cols"]:
                ...
                # No min/max limits – you can type any number
    ...
    if submitted:
        try:
            ...
            # Optional confidence (single number, no JSON)
...
def main():
    ...
    st.markdown(
        """
        Upload a loan dataset, train a model, and predict whether
        a new applicant's loan will be approved.

        **Expected target column:** `Loan_Status` with values **Yes / No**.
        """
    )
    ...
    if uploaded_file is not None:
        ...
        # Prefer Loan_Status if present
        if "Loan_Status" in df.columns:
            target_col = "Loan_Status"
            st.sidebar.success("Using `Loan_Status` as target.")
        else:
            st.sidebar.warning("`Loan_Status` not found. Select target manually.")
        ...
        st.sidebar.markdown("---")
        train_button = st.sidebar.button("Run Full Pipeline")
